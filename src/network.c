...
void reset_network_state(network *net, int b)
{
    ...
    for (i = 0; i < net->n; ++i) {
        ...
        layer l = net->layers[i];
        if(l.state_cl){
            fill_cl(l.outputs, 0, l.state_cl, 1,l.outputs*b);
        }
        if(l.h_cl){
            fill_cl(l.outputs, 0, l.h_cl, 1,l.outputs*b);
        }
        #endif
    }
}
...
{
    network net = *netp;
    cl_push_array(net.input_cl, net.input, net.inputs*net.batch,0);
    if(net.truth){
        cl_push_array(net.truth_cl, net.truth, net.truths*net.batch,0);
    }
    ...
    for(i = 0; i < net.n; ++i){
        ...
        layer l = net.layers[i];
        if(l.delta_cl){
            fill_cl(l.outputs * l.batch, 0, l.delta_cl, 1,0);
        }
        l.forward_cl(l, net);
...
{
    layer l = get_network_output_layer(net);
    cl_pull_array(l.output_cl, l.output, l.outputs*l.batch,0);
}
#endif