...
void forward_local_layer_cl(const local_layer l, network net)
{
    ...
    for(i = 0; i < l.batch; ++i){
        copy_cl(l.outputs, l.biases_cl, 1, l.output_cl, 1,0,i*l.outputs);
    }

    for(i = 0; i < l.batch; ++i){
        cl_mem input = net.input_cl;
        im2col_cl(input, l.c, l.h, l.w, 
                l.size, l.stride, l.pad, net.workspace_cl,i*l.w*l.h*l.c,0);
        cl_mem output = l.output_cl;
        for(j = 0; j < locations; ++j){
            cl_mem a = l.weights_cl;
            int offset_a=j*l.size*l.size*l.c*l.n;
            cl_mem b = net.workspace_cl;
            int offset_b=j;
            cl_mem c = output;
            int offset_c=j+ i*l.outputs;

            int m = l.n;
            int n = 1;
            int k = l.size*l.size*l.c;

            gemm_cl(0,0,m,n,k,1,a,k,b,locations,1,c,locations,offset_a,offset_b,offset_c);
        }
    }
...
void backward_local_layer_cl(local_layer l, network net)
{
    ...
    for(i = 0; i < l.batch; ++i){
        axpy_cl(l.outputs, 1, l.delta_cl, 1, l.bias_updates_cl, 1,i*l.outputs,0);
    }
    ...
    for(i = 0; i < l.batch; ++i){
        ...
        for(j = 0; j < locations; ++j){ 
            cl_mem a = l.delta_cl;
            int offset_a=i*l.outputs + j;
            cl_mem b = net.workspace_cl;
            int offset_b=j;
            cl_mem c = l.weight_updates_cl;
            int offset_c=j*l.size*l.size*l.c*l.n;

            int m = l.n;
            int n = l.size*l.size*l.c;
            int k = 1;

            gemm_cl(0,1,m,n,k,1,a,locations,b,locations,1,c,n,offset_a,offset_b,offset_c);
        }
        ...
        if(net.delta_cl){
            for(j = 0; j < locations; ++j){ 
                ...
                gemm_cl(1,0,m,n,k,1,a,m,b,locations,0,c,locations,offset_a,offset_b,offset_c);
            }

            col2im_cl(net.workspace_cl, l.c,  l.h,  l.w,  l.size,  l.stride, l.pad, net.delta_cl,0,i*l.c*l.h*l.w);
        }
    }
}
...
void update_local_layer_cl(local_layer l, update_args a)
{
    ...
    axpy_cl(size, -decay*batch, l.weights_cl, 1, l.weight_updates_cl, 1,0,0);
    axpy_cl(size, learning_rate/batch, l.weight_updates_cl, 1, l.weights_cl, 1,0,0);
    scal_cl(size, momentum, l.weight_updates_cl, 1,0);
}
...
void pull_local_layer(local_layer l)
{
    int locations = l.out_w*l.out_h;
    int size = l.size*l.size*l.c*l.n*locations;
    cl_pull_array(l.weights_cl, l.weights, size,0);
...
void push_local_layer(local_layer l)
{
    int locations = l.out_w*l.out_h;
    int size = l.size*l.size*l.c*l.n*locations;
    cl_push_array(l.weights_cl, l.weights, size,0);
