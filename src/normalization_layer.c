...
void forward_normalization_layer_cl(const layer layer, network net)
{
    ...
    int h = layer.h;
    int c = layer.c;
    scal_cl(w*h*c*layer.batch, 0, layer.squared_cl, 1,0);

    for(b = 0; b < layer.batch; ++b){
        cl_mem squared = layer.squared_cl;
        int offset_squared = w*h*c*b;
        cl_mem norms   = layer.norms_cl;
        int offset_norms = w*h*c*b;
        cl_mem input   = net.input_cl;
        int offset_input = w*h*c*b;
        pow_cl(w*h*c, 2, input, 1, squared, 1,offset_input,offset_squared);

        const_cl(w*h, layer.kappa, norms, 1,offset_norms);
        for(k = 0; k < layer.size/2; ++k){
            axpy_cl(w*h, layer.alpha, squared, 1, norms, 1,offset_squared,offset_norms);
        }

        for(k = 1; k < layer.c; ++k){
            copy_cl(w*h, norms, 1, norms, 1,offset_norms+w*h*(k-1),offset_norms+w*h*k);
            int prev = k - ((layer.size-1)/2) - 1;
            int next = k + (layer.size/2);
            if(prev >= 0)      axpy_cl(w*h, -layer.alpha, squared, 1, norms, 1,offset_squared+w*h*prev,offset_norms+w*h*k);
            if(next < layer.c) axpy_cl(w*h,  layer.alpha, squared, 1, norms, 1,offset_squared+w*h*next,offset_norms+w*h*k);
        }
    }
    pow_cl(w*h*c*layer.batch, -layer.beta, layer.norms_cl, 1, layer.output_cl, 1,0,0);
    mul_cl(w*h*c*layer.batch, net.input_cl, 1, layer.output_cl, 1,0,0);
}
...
void backward_normalization_layer_cl(const layer layer, network net)
{
    ...
    int h = layer.h;
    int c = layer.c;
    pow_cl(w*h*c*layer.batch, -layer.beta, layer.norms_cl, 1, net.delta_cl, 1,0,0);
    mul_cl(w*h*c*layer.batch, layer.delta_cl, 1, net.delta_cl, 1,0,0);
}
#endif