...
void pull_softmax_layer_output(const softmax_layer layer)
{
    cl_pull_array(layer.output_cl, layer.output, layer.inputs*layer.batch,0);
}

void forward_softmax_layer_cl(const softmax_layer l, network net)
{
    if(l.softmax_tree){
        softmax_tree(net.input_cl, 1, l.batch, l.inputs, l.temperature, l.output_cl, *l.softmax_tree,0,0);
        /*
        int i;
        int count = 0;
        for (i = 0; i < l.softmax_tree->groups; ++i) {
        ...
        */
        ...
        if(l.spatial){
            softmax_cl(net.input_cl, l.c, l.batch*l.c, l.inputs/l.c, l.w*l.h, 1, l.w*l.h, 1, l.output_cl,0,0);
        }else{
            softmax_cl(net.input_cl, l.inputs/l.groups, l.batch, l.inputs, l.groups, l.inputs/l.groups, 1, l.temperature, l.output_cl,0,0);
        }
    }
    if(net.truth && !l.noloss){
        softmax_x_ent_cl(l.batch*l.inputs, l.output_cl, net.truth_cl, l.delta_cl, l.loss_cl,0,0,0,0);
        if(l.softmax_tree){
            mask_cl(l.batch*l.inputs, l.delta_cl, SECRET_NUM, net.truth_cl, 0,0,0);
            mask_cl(l.batch*l.inputs, l.loss_cl, SECRET_NUM, net.truth_cl, 0,0,0);
        }
        cl_pull_array(l.loss_cl, l.loss, l.batch*l.inputs,0);
        l.cost[0] = sum_array(l.loss, l.batch*l.inputs);
    }
}
...
{
    axpy_cl(layer.batch*layer.inputs, 1, layer.delta_cl, 1, net.delta_cl, 1,0,0);
}
