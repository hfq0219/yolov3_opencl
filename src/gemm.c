        ...
        int offset_a,int offset_b,int offset_c)
{
    ...
    *clKernel=clCreateKernel(*clProgram, kernelFun, &err);
    ...
    err|=clSetKernelArg(*clKernel, 10, sizeof(cl_int), &offset_a);
    err|=clSetKernelArg(*clKernel, 11, sizeof(cl_int), &offset_b);
    err|=clSetKernelArg(*clKernel, 12, sizeof(cl_int), &offset_c);
...
void gemm_cl(int TA, int TB, int M, int N, int K, float ALPHA, 
        ...
        int offset_a,int offset_b,int offset_c)
{
    scal_cl(M*N,BETA,C,1,offset_c);
    
    if(!TA && !TB)
        gemm_xx_cl(M, N, K, ALPHA,A,lda, B, ldb,C,ldc,"gemm_nn_opencl",offset_a,offset_b,offset_c);
    else if(TA && !TB)
        gemm_xx_cl(M, N, K, ALPHA,A,lda, B, ldb,C,ldc,"gemm_tn_opencl",offset_a,offset_b,offset_c);
    else if(!TA && TB)
        gemm_xx_cl(M, N, K, ALPHA,A,lda, B, ldb,C,ldc,"gemm_nt_opencl",offset_a,offset_b,offset_c);
    else
        gemm_xx_cl(M, N, K, ALPHA,A,lda, B, ldb,C,ldc,"gemm_tt_opencl",offset_a,offset_b,offset_c);
}
#endif
#ifdef GPU
