...
{
    copy_cl(l.batch*l.inputs, net.input_cl, 1, l.output_cl, 1,0,0);
    int b, n;
    for (b = 0; b < l.batch; ++b){
        for(n = 0; n < l.n; ++n){
            int index = entry_index(l, b, n*l.w*l.h, 0);
            activate_array_cl(l.output_cl, 2*l.w*l.h, LOGISTIC,index);
            if(l.coords > 4){
                index = entry_index(l, b, n*l.w*l.h, 4);
                activate_array_cl(l.output_cl, (l.coords - 4)*l.w*l.h, LOGISTIC,index);
            }
            index = entry_index(l, b, n*l.w*l.h, l.coords);
            if(!l.background) activate_array_cl(l.output_cl,l.w*l.h, LOGISTIC,index);
            index = entry_index(l, b, n*l.w*l.h, l.coords + 1);
            if(!l.softmax && !l.softmax_tree) activate_array_cl(l.output_cl, l.classes*l.w*l.h, LOGISTIC,index);
        }
    }
    if (l.softmax_tree){
        int index = entry_index(l, 0, 0, l.coords + 1);
        softmax_tree(net.input_cl, l.w*l.h, l.batch*l.n, l.inputs/l.n, 1, l.output_cl, *l.softmax_tree,index,index);
    } else if (l.softmax) {
        int index = entry_index(l, 0, 0, l.coords + !l.background);
        softmax_cl(net.input_cl, l.classes + l.background, l.batch*l.n, l.inputs/l.n, l.w*l.h, 1, l.w*l.h, 1, l.output_cl,index,index);
    }
    if(!net.train || l.onlyforward){
        cl_pull_array(l.output_cl, l.output, l.batch*l.outputs,0);
        return;
    }

    cl_pull_array(l.output_cl, net.input, l.batch*l.inputs,0);
    forward_region_layer(l, net);
    //cuda_push_array(l.output_gpu, l.output, l.batch*l.outputs);
    if(!net.train) return;
    cl_push_array(l.delta_cl, l.delta, l.batch*l.outputs,0);
}
...
void backward_region_layer_cl(const layer l, network net)
{
    ...
    for (b = 0; b < l.batch; ++b){
        for(n = 0; n < l.n; ++n){
            int index = entry_index(l, b, n*l.w*l.h, 0);
            gradient_array_cl(l.output_cl, 2*l.w*l.h, LOGISTIC, l.delta_cl,index,index);
            if(l.coords > 4){
                index = entry_index(l, b, n*l.w*l.h, 4);
                gradient_array_cl(l.output_cl, (l.coords - 4)*l.w*l.h, LOGISTIC, l.delta_cl,index,index);
            }
            index = entry_index(l, b, n*l.w*l.h, l.coords);
            if(!l.background) gradient_array_cl(l.output_cl,l.w*l.h, LOGISTIC, l.delta_cl,index,index);
        }
    }
